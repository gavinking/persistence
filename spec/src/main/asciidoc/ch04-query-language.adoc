...
A _functional expression_ is an invocation of a function.

- The Jakarta Persistence query language includes the built-in functions
  described below in <<a5264>>, <<a5284>>, and <<a5304>>. These functions
  may be used in the SELECT, WHERE, or HAVING clause of a query. Use of
  these functions is portable between databases and between providers.

- <<a5311>> describes how predefined native database functions and
  user-defined database functions may be used from within JPQL. Such
  functions are not portable between databases.

If the value of any argument to a functional expression is null or unknown,
the value of the functional expression is unknown.

====== String Functions [[a5264]]

String functions accept or return strings. In Jakarta Persistence, string
functions use 1-based indexing, like SQL, and unlike Java.
...
The CONCAT function accepts an arbitrary-length list of argument strings,
and returns a string that is the concatenation of its arguments.

The SUBSTRING function returns a substring of its first argument.
The second and third arguments specify the starting position and
...
from the start position to the end of the string is returned. The first
position of a string is denoted by 1.

The TRIM function trims the specified character from a string. The
optional `trim_specification`, if any, determines whether characters
should be trimmed from the start of the string, the end of the string,
or both. If a trim specification is not specified, it defaults to BOTH.
The optional `trim_character` is a single-character string literal or
a character-valued input parameter (i.e., `char` or `Character`).
footnote:[Note that not every database supports the use of a trim
character other than the space character; use of this argument may
result in queries which are not portable between databases.] If the
character to be trimmed is not explicitly specified, it is be assumed
to be space (or blank). The TRIM function returns the trimmed string.

The LOWER and UPPER functions convert a string to lower and upper case,
respectively, according to the locale of the database. They return a
string.
...
The LENGTH function returns the length of the argument string measured in
characters as an integer.

====== Arithmetic Functions [[a5284]]

Arithmetic functions accept or return numeric values.

- The basic types `long`, `int`, `short`, `byte`, `Long`, `Integer`,
 `Short`, `Byte`, and `BigInteger` are considered integer numeric types.

- The basic types `float`, `double`, `Float`, `Double`, and `BigDecimal`
  are considered non-integer numeric types.
...
The ABS (absolute value), CEILING, and FLOOR functions accept a numeric
argument and return a number (integer, float, or double) of the same
numeric type as the argument.

The SIGN function accepts a numeric argument and returns an integer,
either `0`, `1`, or `-1`, according to whether its argument is zero,
positive, or negative.

The SQRT (positive square root), EXP (exponential), and LN (natural
logarithm) functions accept a numeric argument and return a double.

The MOD (integer division) function accepts two integer arguments and
returns an integer.

The ROUND function accepts a numeric argument and an integer argument
and returns a number of the same type as the first argument. The
second argument specifies the maximum number of significant digits to
the right of the decimal point.

The POWER function accepts two numeric arguments and returns a double.
The first argument is the base, and the second number is the exponent.

The SIZE function returns an integer value, the number of elements of
the collection. If the collection is empty, the SIZE function evaluates
to zero.

The INDEX function returns an integer value identifying the position of
its argument in an ordered list. The INDEX function can only be applied
to identification variables denoting elements of a list for which an
order column has been specified.

In the following example, _studentWaitlist_ is a list of students for
