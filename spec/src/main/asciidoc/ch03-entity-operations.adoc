...
The entity instance lifecycle is defined in <<lifecycle>>. The relationship
between entity managers and persistence contexts is described in
<<context-lifetime-transaction>>, and again in further detail in <<a11431>>.
...
remove persistent entity instances, to find persistent entities by
primary key, and to query over persistent entity types. <<entity-manager>>
describes the `EntityManager` interface. <<locking>> describes mechanisms
for concurrency control and locking. <<a4639>> provides a summary of
exceptions.
...
to react to events occurring within the persistence context. <<callbacks>>
...
=== EntityManager Interface [[entity-manager]]
...
entity instances in which for any given _persistent entity identity_,
defined by an entity type and a primary key value, as specified in
<<identity>> and <<managed-entities>>, there is at most one entity instance.
...
<<persistence-context-type>> and <<a11431>>.

=== Entity Instance Life Cycle [[lifecycle]]
...
as defined in <<relationships>>, specifies which lifecycle operations
propagate to associated entities. Each lifecycle operation is identified
by the corresponding member of the enumeration `CascadeType`.
...
==== Detached Entities [[detached-entities]]
...
- the persistence context is a container-managed transaction-scoped
  persistence context, as defined in <<persistence-context-type>>,
  and the transaction associated with the persistence context commits,
  as specified in <<commit>>,
...
If `e` has a version, as defined in <<version>>, the persistence provider
...
- For an `EntityManager`, every entity instance returned by any
  ...
  or `clear()`, as specified above in <<detached-entities>>. An entity
  manager is required to ensure that if an entity instance representing
  a given record already exists in its associated persistence context,
  then exactly that instance is returned by every operation that returns
...
described above in <<detached-entities>>.

An attribute of an entity or embedded class might also be considered _loaded_.

- An embedded attribute, as defined in <<embeddable>> is considered to be
  loaded if the embedded attribute was loaded from the database or assigned
  by the application, and, if the attribute references a non-null instance,
  the embeddable instance is loaded.
...
=== Persistence Context Lifetime and Transaction Synchronization [[context-lifetime-transaction]]
...
==== Persistence Context Types [[persistence-context-type]]
...
Persistence context synchronization types are further described in
<<a11797>>.
...
=== Locking and Concurrency [[locking]]
...
modes. Optimistic locking is described in <<optimistic-locking>> and
<<optimistic-version>>; pessimistic locking in <<pessimistic-locking>>.
<<lock-modes>> describes the setting of optimistic and pessimistic lock
modes. The configuration of the setting of optimistic lock modes is
described in <<a2100>>, and the configuration of the setting of
pessimistic lock modes is described in <<a2113>>.
...
automatically for every entity with a version, as defined in <<version>>.
...
- every relationship owned by the entity, as defined by <<relationships>>,
...
==== Entity Versions and Optimistic Locking [[optimistic-version]]
...
locking. See <<optimistic-version>> and <<lock-modes>>.
...
=== Entity Listeners and Callback Methods [[callbacks]]
...
==== Semantics of the Life Cycle Callback Methods for Entities [[entity-callbacks]]
...
pre-remove entity lifecycle events described in <<entity-callbacks>>.
...
- a basic type (see <<basic-types>>) used as an intermediate step in mapping to
...
See <<context-lifetime-transaction>> for information about transaction-scoped
and extended persistence contexts. See <<a11431>> for further discussion of
entity manager use with no active transaction and persistence context types.
...
- the result class is the class of a basic type listed in <<basic-types>>
  and the result set must have a single column which is interpreted as a
