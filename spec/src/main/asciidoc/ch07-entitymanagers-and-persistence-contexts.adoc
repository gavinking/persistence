...
The `EntityManagerFactory` interface may be found in <<_entitymanagerfactory_>>.

An `EntityManagerFactory` is used
...
An `EntityManagerFactory` provides
...
The `EntityTransaction` interface may be found in <<_entitytransaction_>>.

An instance of `EntityTransaction` is used to
...
The `PersistenceUnitUtil` interface may be found in <<_persistenceunitutil_>>.
