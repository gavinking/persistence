...
=== Persistent Fields and Properties [[attributes]]
...
- any basic type listed below in <<basic-types>>, including any Java
  `enum` type,
- an entity type or a collection of some entity type, as specified in
  <<relationships>>,
- an embeddable class, as defined in <<embeddable>>, or
- a collection of a basic type or embeddable type, as specified in
  <<element-collection>>.
...
hierarchy is determined by the rules defined in <<access>>.
...
- an attribute of **basic type**, as defined in <<basic-types>>, or
  of a type converted to a basic type through use of a _converter_,
  as defined in <<a2999>>,
- an **embedded attribute** whose type is a fine-grained embeddable
  class, as defined <<embeddable>>,
- an **element collection**, that is, a collection whose elements are
  instances of a basic type or embeddable class, as defined in
  <<element-collection>>,
- a **relationship** (or **association**) with another entity type,
  as defined in <<relationships>>, which is further classified by its
...
the defaulting rules specified below in <<attribute-mapping-defaults>>
...
the entity class, as specified above in <<attributes>>. `AccessType`
...
==== Explicit Access Type [[explicit-access]]
...
=== Primary Keys and Entity Identity [[identity]]
...
of the `EntityManager` or `EntityAgent`, as described in <<entity-operations>>,
and represents the _persistent identity_ of the instance.
...
- The access type (`FIELD` or `PROPERTY`) of a primary key class is
  ...
  key, unless the primary key is an embedded id and an explicit access
  type is specified using the `Access` annotation, as defined in
  <<explicit-access>>.
...
- The primary key class must define `equals` and `hashCode` methods. The
  ...
  the database equality for the database types to which the key is mapped.
...
- A primary key which involves a derived identity must conform to the
  rules specified below in <<derived-identity>>.

==== Primary Keys Involving Derived Identities [[derived-identity]]
...
composite primary key, as described in <<derived-identity-specification>>
below. The dependent entity class has a composite primary key if
...
===== Specification of Derived Identities [[derived-identity-specification]]
...
- Any primary key attribute of a dependent entity which is derived from the
  ...
  this relationship is specified in <<relationship-mapping-defaults>>. The
  default mapping may be overridden by annotating the relationship attribute
  with the `JoinColumn` or `JoinColumns` annotation.
...
=== Entity Versions [[version]]

An entity might have a _version_, a persistent field or property used by the
persistence provider to perform _optimistic lock verification_, as specified
in <<optimistic-version>>. The version field or property holds a version
number or timestamp identifying the revision of the entity data held by an
entity class instance. In the course of performing lifecycle operations
involving the entity instance, the persistence provider gets and sets the
version field or property of the entity instance to determine or modify its
version number or timestamp. The `Version` annotation defined in <<a16432>>
or`version` XML element must be used to explicitly identify the version field
...
object is determined as described in <<access>>.

An embeddable class may be a regular Java class that adheres to the
requirements specified in <<entity>> for entities, with the exception
that an embeddable class is not annotated as `Entity`, and an embeddable
...
=== Collections of Embeddable Classes and Basic Types [[element-collection]]

An _element collection_ is a persistent field or property of an
entity or embeddable class that is collection-valued, and whose
elements are of a basic type, as specified above in <<basic-types>>,
or of an embeddable class, as specified immediately above in
<<embeddable>>.
...
<<attribute-mapping-defaults>> apply.
...
- but it may have a many-to-one or one-to-one association to an
  ...
  mapping, as defined in <<relationships>>.
...
In this case, default mappings are described in <<relationship-mapping-defaults>>.
...
=== Mapping Defaults for Non-Relationship Fields or Properties [[attribute-mapping-defaults]]
...
2. Otherwise, if the type of the field or property is one of the basic
   types listed in <<basic-types>>, it is treated as if it were annotated as
...
This is described in <<relationship-mapping-defaults>>.
...
<<relationship-mapping-defaults>> specifies default mappings for entity
relationships. These defaulted mappings may be customized through the
use of mapping annotations or via an XML descriptor. Furthermore, this
specification requires support for the following additional mapping
strategies:
...
as specified later in <<lifecycle>>. An attribute annotated `ManyToOne` or
...
=== Relationship Mapping Defaults [[relationship-mapping-defaults]]

This section defines the mapping defaults that apply to entity associations
declared using the relationship modeling annotations defined above in
<<relationships>>. These mapping defaults also apply when the XML descriptor
is used to declare a relationship.
...
==== Bidirectional ManyToMany Relationships [[bidirectional-relationships]]
